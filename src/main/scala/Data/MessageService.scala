...
class MessageImpl extends MessageService:
  ...
  // A message contains : username, msgContent, mention, exprType, replyToId
  val messages = scala.collection.mutable.Map[
    Long,
    (Username, MsgContent, Option[Username], Option[ExprTree], Option[Long])
  ]()
  ...
  override def add(
    ...
    messages(id) = (sender, msg, mention, exprType, replyToId)
    id

  override def getLatestMessages(n: Int) =
    val lastN = messages.toSeq.sortBy(_._1).takeRight(n)
    lastN.map((x) => (x._2._1, x._2._2))
